...
def compose_task_name(job_name:str, task_name: str) -> str:
    ...
    if len(name) > 32:
        raise ValueError("Task name too long! The length of `job_name` plus the length of `task_name` must within `31`")
    return name 
...
def compose_command(file:str) -> str:
    ...
    try:
        ...
        raise Exception(f"Wrong file name `{file}`, proper file name should have suffix, e.g, `.py`, `.ipynb`, etc")
...
def compose_command_for_job(file, to_run_new:bool) -> str:
    if to_run_new:
        return get_enviroment_set_command_by_platform("PYDAG_RUN_NEW", "yes") + " && "+ compose_command(file)
    else:
        return get_enviroment_set_command_by_platform("PYDAG_RUN_NEW", "no") + " && "+ compose_command(file)
