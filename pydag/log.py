...
class BasicJobLogger:
    ...
    def log_job(self, job_name, latest_n=3):
        ...
        record_df.columns = ["job_name", "job_run_at", "task_name", "run_id", "task_id", "status", "task_record_at"]
        record_df= record_df.sort_values(["job_run_at", "task_record_at"], ascending=[False, True])
        job_run_ats = record_df["job_run_at"].unique()[:latest_n]
        record_df = record_df[record_df["job_run_at"].isin(job_run_ats)] 
        
        for job_run_at, df in record_df.groupby("job_run_at"):  #  for log, time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(_job_start_at))
...
def display_job_run(job_name, job_run_at, tasks, job_graph):
    ...
    build_log_tree(tree, job_graph, root_task_name, tasks)

    rprint(tree)


def build_log_tree(tree, job_graph, root_task_name, tasks, method="dependant"):
    if method == "dependant":
...
def build_dependant_log_tree(tree, graph, root, tasks):
    task_info = [task for task in tasks if task["task_name"]==root][0]  # get self task details
